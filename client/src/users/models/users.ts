...
// this model represents the the ngrx store. It has a matching reducer i.e.
...
export class Users {
    ...
    constructor(private _store:Store<any>, api:ApiService) {
        ...
        // the data is an Immutable Map so we have to use a getter
        this.adding$ = usersStore$.map(data => data.get('adding'));

        // the data is an Immutable Map so we have to use a getter
        this.loading$ = usersStore$.map(data => data.get('loading'));

        // stream of arrays which contain user Records
        this.users$ = usersStore$.map(data => {
            return data.get('result').reduce((acc, userId) => {
                // immutable.getIn converts array to object path i.e. return data at entities/users/3
        ...
        // 'action' comes from the original observable and the payload:IUser comes
        // from the merged observable i.e. api.createUser().
        // the value returned from the function is passed to its subscribers.
        //
        // 'action' object is created in this.actions$.next({type: ADD_USER, payload: user});
        // 'payload' is the response from the call to api.createUser(action.payload) http request
        // the return object {type: ADDED_USER, payload} is the value returned from the callback
        // that return action action is then dispatched to the store here...
